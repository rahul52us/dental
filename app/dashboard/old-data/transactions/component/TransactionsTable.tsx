...
const TransactionsTable = observer(() => {
  ...
  const handlePreviewReport = async () => {
    setIsPrinting(true);
    const base64 = await oldDataStore.generateTransactionReportBase64();
    setIsPrinting(false);
    if (base64) {
      setPdfBase64(base64);
      onPreviewOpen();
  ...
  const columns = [
    { headerName: t("Date"), function: (row: any) => new Date(row.date).toLocaleDateString('en-IN') },
    ...
    { headerName: t("Fee Received"), function: (row: any) => <Text fontWeight="bold" color="green.500">₹{row.fee_rec}</Text> },
    {
  ...
  return (
    <Box p={4}>
      ...
      <ReceiptPreviewDrawer
        ...
        pdfBase64={pdfBase64}
        fileName="historical_transactions_report.pdf"
      />
    </Box>
  );
});
