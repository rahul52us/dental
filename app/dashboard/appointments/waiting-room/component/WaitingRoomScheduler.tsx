                        ...
                        {chairs.map((chair: any) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* ➕ Add Appointment (ONLY if slot is allowed & empty & NOT disabled) */}
                                    {!occupied && allowed && !disableBooking && (
                                        <Flex
                                            position="absolute"
                                            inset={0}
                                            align="center"
...
export default function WaitingRoomScheduler({
    handleTimeSlots,
    selectedDate,
    setSelectedDate,
    ...
    const fetchAppointments = async () => {
        ...
        const res = await getChairSummary({
            ...
            status: filterStatus // Pass status filter to API
        });
        ...
        if (res?.status === "success") {
            ...
            res.data.forEach((chair: any) => {
                ...
                chair.appointments.forEach((apt: any) => {
                    // Frontend filtering removed as backend handles it
                    ...
                    allAppointments.push({
                        ...
                        chairId: chair._id,
                        status: apt.status, // Ensure status is preserved
                    });
                });
            });
    ...
    useEffect(() => {
        fetchAppointments();
    }, [selectedDate, getChairSummary, createdAppointmentByCalender, filterStatus]);
