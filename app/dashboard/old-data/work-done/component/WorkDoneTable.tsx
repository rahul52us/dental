...
const WorkDoneTable = observer(() => {
  ...
  const handlePreviewReport = async () => {
    setIsPrinting(true);
    const base64 = await oldDataStore.generateWorkCompReportBase64();
    setIsPrinting(false);
    if (base64) {
      setPdfBase64(base64);
      onPreviewOpen();
  ...
  const columns = [
    { headerName: t("Date"), function: (row: any) => new Date(row.wrk_date).toLocaleDateString('en-IN') },
  ...
  return (
    <Box p={4}>
      ...
      <ReceiptPreviewDrawer
        ...
        pdfBase64={pdfBase64}
        fileName="historical_work_done_report.pdf"
      />
    </Box>
  );
});
